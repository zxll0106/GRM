...
class VolleyballDataset(data.Dataset):
    ...
    """
    def __init__(self,anns,tracks,frames,images_path,image_size,feature_size,inference_module_name,cfg,num_boxes=12,num_before=4,num_after=4,is_training=True,is_finetune=False):
        self.anns=anns
        self.tracks=tracks
        self.frames=frames
    ...
    def volley_frames_sample(self,frame):
        ...
        if self.is_finetune:
            if self.is_training:
                ...
                #         range(src_fid - self.num_before, src_fid + self.num_after + 1)]
            else:
                return [(sid, src_fid, fid)
